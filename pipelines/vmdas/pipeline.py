...
class VmdasADCP(IngestPipeline):
    def hook_customize_dataset(self, dataset: xr.Dataset) -> xr.Dataset:
        ...
        if (
            ...
            warnings.warn(
                f"Using heading misalignment of {offset}. Update vmdas/pipeline.py as necessary."
            ...
            dataset["heading_gps"].values = ss.medfilt(dataset["heading_gps"], 5)
            dataset["heading"] = (
        ...
        # Correct velocity with GPS or with both GPS and BT
            ...
            # Otherwise calculate GPS velocity
                ...
                if "speed_over_grnd_gps" in dataset:
                    # Calc vel E and vel N
                    dataset["dir_over_grnd_gps"].values = ss.medfilt(
                        dataset["dir_over_grnd_gps"], 5
                    )
    ...
    def hook_plot_dataset(self, ds: xr.Dataset):
        ...
        with plt.style.context("shared/styling.mplstyle"):
            ...
            ax[0].plot(ds["time"].values, ds["vel"][0].mean("range"), label="East")
            ...
            for a in ax:
                a.grid("x")
            plot_file = self.get_ancillary_filepath(title="depth-avg")
            fig.savefig(plot_file)
            plt.close(fig)
